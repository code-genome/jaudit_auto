...
dl_dir=config['*'].get('downloader_dir',None)

if dl_dir is None:
    sys.stderr.write(f"downloader_dir is not set in {config_file}\n")
    error = True
...
if dl_dir[0] != '/':
    dl_dir = os.path.join(basedir, dl_dir)
...
for id in config:
    ...
    downloader=config[id].get('downloader',None)
    if downloader is None:
        sys.stderr.write(f"No downloader specified for {id}, skipping.\n")
        continue
    ...
    args=' '.join(["'"+a+"'" for a in config[id].get('args',None).split(' ')])

    if repo[0] != '/':
        repo = os.path.join(data_dir, repo)

    if downloader[0] != '/':
        downloader = os.path.join(dl_dir, downloader)

    os.system(f"{downloader} '{repo}' '{archive}' {args}")
